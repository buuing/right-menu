...
- [x] `🎯 v2.0.2 新增` ~菜单的宽度根据文字长度做到自适应~
- [x] `🎯 v2.0.2 新增` ~增加不同的主题样式: mac / win10, 看看能不能根据系统自动切换主题~
- [ ] `📆 进行中` 目前最多支持二级菜单, 后续增加三级菜单 / 多级菜单的逻辑
- [ ] `📆 进行中` 增加初始化高级配置, 可以设置一些全局的选项, 以及菜单创建的生命周期
- [ ] `📆 进行中` 增加class选项, 可以给当前标签添加类样式
